...
func NewConfig(admissionV1Enabled, namespaceSelectorEnabled bool) Config {
	return Config{
		...
		validationEnabled:        config.Datadog().GetBool("admission_controller.validation.enabled"),
		mutationEnabled:          config.Datadog().GetBool("admission_controller.mutation.enabled"),
