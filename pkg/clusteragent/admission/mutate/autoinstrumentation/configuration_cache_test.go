...
func TestDisableConfiguration(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "Disable the only remote configuration",
			localConfig: ssiConfig{
				enabled: false,
			},
		...
		{
			name: "Disable last applied remote configuration",
			localConfig: ssiConfig{
				enabled: false,
			},
		...
		{
			name: "Disable first applied remote configuration",
			localConfig: ssiConfig{
				enabled: false,
			},
			currentConfig:    ssiConfig{enabled: true, enabledNamespaces: []string{"ns1", "ns2"}},
...
func TestUpdate(t *testing.T) {
	tests := []struct {
		...
		{
			name: "Multiple rules: enable and disable instrumentation",
			localConfig: ssiConfig{
				enabled: false,
			},
			remoteRequests: []Request{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			enabledNamespaces := tt.localConfig.enabledNamespaces
			disabledNamespaces := tt.localConfig.disabledNamespaces
			c := newInstrumentationConfigurationCache(nil, &enabled, &enabledNamespaces, &disabledNamespaces, tt.clusterName)
